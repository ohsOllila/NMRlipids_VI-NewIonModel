...
\begin{abstract}
  ...
  consistently determined with experimental and computational methods,
  despite their significance and biological relevance in cellular membranes. 
  The details concerning ion binding could be resolved with classical molecular dynamics (MD) simulations,
  however, the accuracy of the available lipid models has not been sufficient.
  In this work we show that the binding details of Na$^+$ and Ca$^{2+}$ ions to the
  1-Palmitoyl-2-oleoylphosphatidylcholine (POPC) bilayer
  can be accurately described with a MD simulation model implicitly including electronic polarization.
  This is demonstrated by applying the electronic continuum correction (ECC) to a
  state of the art lipid model for MD simulations of the POPC
  lipid bilayer. The newly introduced ECC-lipids model reproduces the experimentally measured structural parameters for the ion-free membrane,
  the response of lipid headgroup to a bound positive charge, and the binding affinities of
  Na$^+$ and Ca$^{2+}$ ions. The observed negligible binding of Na$^+$ ions to POPC bilayer
  and stronger interactions of Ca$^{2+}$ primarily with phoshpate oxygens give
  support to the early intepretations of the experimental spectroscopic data.
  At the same time, Ca$^{2+}$ ions form complexes with 1-3 lipid molecules with almost
  equal probabilities, suggesting a more complicated binding stoichimetry than 
  the ternary complex binding model used to interpret the NMR data.
  The results of this work pave the way to accurate MD simulations of complex biochemical systems
...
the RESP charges may be subsequently modified to fit certain experimental observables \todo{This needs a citation - Joe please pick 1-2 papers}. 
...
Such a setup and the above definition of bulk ion concentration provide a simple relation for the normalized surface excess $\Gamma_i^w$ for simulations of lipid bilayers, 
...
For comparison, simulation with unscaled charges were perfomed using the Lipid14 model used the TIP3p water model \cite{jorgensen83} and the ion model by
...
due to its overal very weak binding affinity.
...
This probably explains why the simple the ternary complex model fitts the
...
the ECC-lipids model with existing models for proteins and other biological molecules should to be verified and, if necessary, further adjustments along the ECC concept in force fields of molecules interacting with the lipids need to be done.
