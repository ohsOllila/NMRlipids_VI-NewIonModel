...
\subsection{Comparing head group order parameter response to bound charge in simulations and experiments by using the electrometer concept} \label{section:electrometer}
...
The experimental head group order parameter data used
in this work is reported as a function of 
cation concentration remaining in the bulk 
solvent \cite{akutsu81,altenbach84}. 
\todo{JOE: Following is hard to understand 
(even though I think I know the idea. 
Should be rewritten so that it is clear.}
...
\todo{JOE: I wouldn't use the term plateau. Term ``region where concentration is constant'' is preferred. Applies to the whole text -- replace everywhere. }
...
\todo{Were all scaled? Are there charges is tails? Were those scaled as well in this case? 
      -- JOE: yes, all scaled for simplicity. } 
...
the response of head group order parameters to the amount of 
...
locate in the lipid bilayers. Comparison of head group order parameter
...
lipid head group to bound charge in simulation models.
...
The head group order parameter changes as a function of mole fraction
...
head group P-N vector angle with respect to the membrane normal
is shown in Fig.~\ref{OrderParameterCHANGESsurf}.
The head group orients more parallel to the membrane
...
in ECC-lipids model, which is in line with the higher order parameter 
response and reduced charge--dipole interactions in the ECC-lipid model.
...
The linear decrease of the head group order parameters, quantified in previous 
section, can be used to measure the amount of bound aqueous cations in lipid 
bilayers~\cite{seelig87,catte16}. The head group order parameter response 
...
As already discussed previously~\cite{catte16}, NaCl has a negligible effect 
\todo{JOE: catte16 is not the original work that introduces NaCl effects -- on the
           contrary, the presented simulations (which for the core of the original work there)
           rather suggest that there is some effect. 
           I think this has to be started from a different perspective.
           For example, we can start with saying that we use Na and Ca as biologically relevant cations
           and they also are representants of mono- and divalent cations. 
           In addition, they also represent different behaviour accordin to experimental
           measurements of head group order parameter. }
on the head group order parameters with submolar 
concentrations due to the very weak binding in PC bilayers. 
In contrast to almost all the other simulation models, this behaviour 
is reproduced by Lipid14 model with \AA{}qvist ions \cite{catte16}. 
The head group order parameter response to CaCl$_2$ concentration was,
however, overestimated with the same combination of force field
parameters~\cite{catte16}. The results were not improved when
ion models by Dang et al.~\cite{smith94,chang1999,dang2006} or
ECC-ions~\cite{jungwirth17-new-paper-to-be-published, kohagen16, Pluharova2014}
with more realistic bulk behaviour were used, as seen in 
...
Significant improvement is achieved with the newly developed ECC-lipid model. 
The head group order parameter changes as a function of CaCl$_2$
concentration show a good agreement between ECC-lipid model and 
experiments in Fig. \ref{OrderParameterCHANGESnewMODELS}.
Ion binding affinity is quantified by using the relative surface
excess between ions and water from Eq.~\ref{surfexcess} and  
density profiles along membrane normal shown in Fig.~\ref{fig:cacl-dens}.
...
The relative surface excess for ECC-lipid model with ECC-ions $\Gamma_i^w = 0.07 nm^{-3}$
is significantly smaller than for Lipid14 with \AA{}qvist $\Gamma_i^w = 0.13 nm^{-3}$
and Dang ions $\Gamma_i^w = 0.3 nm^{-3}$ (error estimate being below 10\%).
...
\todo{This paragraph is to be finalized when we have the surface excess analysis
  ...
  JOE: Relative surface excess values are there, so let's finalize it.}
...
\begin{figure}[tbp]
  ...
  \caption{\label{fig:binding_conf}
      Configurations of \ce{Ca2+} binding in POPC membranes in the ECC-lipids model.
