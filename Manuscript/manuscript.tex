...
Binding stoichiometry of \ce{Ca2+} and POPC was 
thoroughly studied in the experimental work~\citenum{altenbach84}.
...
In such a model, it is assumed that \ce{Ca2+} cations bind to a POPC membrane 
with a stoichiometry 2~POPC:1~\ce{Ca^{2+}}.
Such a model also provides a good fit 
to our simulations with ECC-lipids 
(see Fig.~\ref{fig:cacl-bind} in SI and its caption for details). 
Simulations, however, allow us to directly evaluate the stoichiometry 
by calculating relative propensities of various \ce{Ca2+}:$n\times$POPC clusters
...
The fractions of \ce{Ca2+}:$n\times$POPC complexes at 285~mM concetration 
are then in order:
$42\%$ for two lipids,
$30\%$ for one lipid, and
$28\%$ for three lipids.
The symmetry of such a distribution --  
...
\begin{figure}[tbp]
  ...
  \todo{JOE: I'll update this figure with some ensemble of configuration to support binding preference of \ce{Ca2+}}
\end{figure}
...
$C_b$, mole fraction of bound \ce{Ca2+} per POPC, and $\sqrt{C_b/C_I}$, 
...
Such an extrapolation is valid as long as the mode of \ce{Ca2+} binding 
...
for divalent cations like \ce{Ca2+}~\cite{Andelmann1995}. 
...
gradually do not hold for higher concentrations of \ce{Ca2+}. 
...
\begin{figure}[]
  ...
  \caption{\label{fig:cacl-bind}
    Ternary complex binding model of \ce{Ca2+} to a POPC membrane 
