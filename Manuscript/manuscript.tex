...
if their distance is less than or equal $0.3 \, \mathrm{nm}$ (robustness of this definition discussed in reference~\citenum{javanainen17}). 
...
This does not change the dominance of phosphates, which still interact more than 6x times more. That's good.
In addition, the probability of states, containing sn-1:O increases from 6\% to 10\%.
...
The slight problem may be that with this tweak, there's approx. 20\%  more cations bound to the membrane,
which means, I'd need to tune the sigma parameter a bit. 
Not $f_q$ -- that's set by the simulations with the surfactant. 
That could detune the membrane structure \dots
I think that's within the accuracy.
...
(Fig. S17 in \citenum{javanainen17} -- C36 with MK Ca2+, any other model, either different cation or Slipids/C36, just such calcium)
this time can be roughly estimated -- get the simulation from \url{https://doi.org/10.5281/zenodo.259376} and analyze it -- 
...
Interestingly, \ce{Na+} cations exhibit much more rapid exchange between membrane and solvent.
