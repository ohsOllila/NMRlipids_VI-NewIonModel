...
%\vspace{0.5cm}
\chemfig{
 -[:0,3.5,,,draw=none]\chemabove{N}{\scriptstyle\oplus} (-[:135]H_3C)(-[:225]H_3C)(-[:315]{(}CH2{)}_{16}-CH_3)(-[:45]{(}CH2{)}_{16}-CH_3)
} , 
\vspace{0.5cm} \\
...
\begin{figure}[tbp]
  ...
  \todo{X-label misses glycerol region. JOE: glycerol belongs to the headgroup part, but, yes, it wouldn't hurt to write it explicitly. SAMULI: I would maybe remove the whole text, tell the information in caption and increase the order parameter figure size in z-direction.}
  \todo{Also acyl chain points should correspond the legends and maybe have the same size as HG and glycerol.}
  \todo{Legends would be more clear without multiple points.}
\end{figure}
...
As in the original Lipid14 model~\cite{dickson14}, the acyl chain order parameters agree
well with the experimental values. Notably,

and in almost all the other available
lipid models \cite{ollila16}

%This is expected as the ECC model does not modify
%the already highly optimized parameters of this region in Lipid14. 
The headgroup order parameters $\alpha$ and $\beta$ and P-N vector angle are slightly larger
in the ECC-lipid model in comparison to Lipid14 (Figs.~\ref{simVSexpNOions} and \ref{OrderParameterCHANGESsurf}). 
