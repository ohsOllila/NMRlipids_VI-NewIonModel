...
Zwitterionic phosphocholine (PC) lipid bilayers are common model systems for
cellular membranes. Interactions of biological cations, especially Na$^+$ and
Ca$^{2+}$ are with 
have been widely used as
model systems to study details of cation interactions with cellular membranes
by using experimental \cite{cevc90,tocanne90,binder02,pabst07,uhrikova08}
...
Note that the area of the interface is given by the simulation box area
multiplied with two $2A_b$, because bilayers have a interface
in both leaflefts.
...
\begin{figure}[tbp]
  ...
  \todo{I would put the acyl chain order parameters also for original Lipid14}\\
...
Approximately linear decrease is observed
...
$\Gamma_i^w = 0.07 nm^{-3}$ for the ECC-lipid model,
