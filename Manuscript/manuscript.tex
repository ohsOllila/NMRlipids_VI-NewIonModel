...
dielectric constant gives values around 2 
...
it is nota priori  clear whether the same factor should be used for partial charges in molecules,
...
Importantly, results from simulations can be directly
compared against experimental NMR data \cite{akutsu81,altenbach84,scherer89},
as discussed in detail in Ref. \citenum{catte16}.
To this end, we explore the above discussed scaling factor parameter space, $f_q \in (0.75, 1.0)$ for the Lipid14~\cite{dickson14} force field.
...
in a recent test within NMRlipids project (see Fig.~5 in Ref.~\citenum{catte16}).
Moreover, the glycerol backbone and head group structures in Lipid14 model were
more realistically described compared with other state of the art lipid models \cite{botan15}.
...
In contrast, we do not modify the hydrocarbon chain parameters, as they
...
parameters in the presence of ions, which call for improvements in all available lipid models~\cite{botan15}. 

Exploring different values of the scaling factor, we found out that ion bindings and the
related head group order parameter responses become weakened in general. 
The optimal behaviour  was observed for a
scaling factor of $f_q = 0.8$.This value is only slighly higher than the original scaling factor of 0.75, which implies
...
While, the above charge scaling improved the description of lipid-ion interactions,
it also led to a reduction of the area per molecule of the lipid bilayer in water below experimental
values. Simulations with Lipid14 parameters having partial charges of head group, glycerol
backbone and carbonyls scaled with 0.8 gave the area per molecule value of $\approx$60~\AA$^2$, as well as
which is somewhat smaller than the experimental value of 64.3~\AA$^2$ (\cite{}\todo{missing REF for APL experiment})
the Lipid14 value of $(65.6 \pm 0.5)$~\AA$^2$~\cite{dickson14}.
...
by reducing the effective radius of atoms with scaled charges by changing the 
the $\sigma$ parameters in the Lennard-Jones potential, as done previously for ions in solution \cite{kohagen14,kohagen16,Pluharova2014}.
...
models were quantified by calculating the relative surface excess using Eq.~\ref{surfexcess} 
and the bulk concentrations determined from the density profiles in Fig.~\ref{fig:cacl-dens}. 
All these simulation have the same, $350\,\mathrm{mM}$, concentration of CaCl$_2$ with 
respect to water (concentration before solvating the lipids in experiments).
...
However, I am not sure if there the table is useful in the end.}
...
were analyzed by calculating the probability of the
contacts with the smaller or equal distance than $0.3 \, \mathrm{nm}$, 
as done previously in Ref.~\citenum{javanainen17}. 
The results are shown in Table \ref{tab:binding}.
%
...
The probability for the bound Ca$^{2+}$ ions to be in contact with the phosphate oxygens
is 99\%, indicating the interactions of Calcium ions purely with carbonyl oxygens has the probability of
less than 1\%. However, the probability for the bound Ca$^{2+}$ ion to be in contact with
carbonyl oxygen is 25\%, indicating that roughly quarter of the bound Calcium ions are in close contact
with both, phosphase and carbonyl oxygens. It should be noted that in this analysis carbonyl and
phospate may or may not belong to the same lipid, and the carbonyl chain locate in either of the 
acyl chains. While the higher concentrations of \ce{CaCl2} naturally increased the amount of contacts per lipid,
...
Even thought the \ce{Na+} ions bind to POPC bilayer with the very low probability,
it is possible to analyze the distribution of contacts from simulations with a large
sodium concetration of $1000\,mM$.
...
The results show that the sodium ions bind purely to the phosphate oxygens 
of POPC with the probability of 55\% and purely to the carbonyl oxygens
with the probability of 20\%, leaving the probability of 25\% for the
...
with the previous conclusions based in large amount of experimental and theoretical
studies \cite{hauser76,hauser78,herbette84,cevc90,binder02}, but slightly updates the
picture given by the previous MD simulation, where the binding to carbonyls was found
to be more pronounced \cite{bockmann03,bockmann04,melcrova16,javanainen17}.
Also the Sodium ions bind most likely to the phosphate oxygens, while the probability
of binding purely to carbonyls is also significant. The physiological relevance
of the details of sodium binding is, however, uncertain due to the very weak binding.
...
The previous interpretation of the experimental data for the changes of lipid headgroup
order parameters, shown in Fig. \ref{fig:delta_ordPar_CaCl}, concluded that
the experimental data was best explained by the ternary complex binding model, 
where \ce{Ca^{2+}} ion bind to a POPC membrane with a stoichiometry 2~POPC:1~\ce{Ca^{2+}}~\cite{altenbach84}.
However, also a Langmuir adsorption model (i.e. stoichiometry 1~POPC:1~\ce{Ca^{2+}}) 
was successfully fit to the experimental data when only low concentrations of \ce{CaCl2} were considered~\cite{macdonald87}.
These binding models and the measured headgroup order parameter changes
are also inluded in the tabulated experimental binding constants~\cite{marsh13}.

Since the experimentally measured changes of the POPC lipid headgroup order parameters
as a function of CaCl$_2$ concentration are reproduced by the ECC-lipid model,
it can be used to give more versatile interepration of binding stoichiometry than
the simple binding models. The stoichiometry was directly evaluated from simulations
by calculating the relative propensities of various \ce{Ca^{2+}}:$n\times$POPC complexes,
which were defined to occur when cations and lipids were found within a cut off distance of $0.3\,\mathrm{nm}$. 
The observed propensities of
$42\%$ for two lipids,
$30\%$ for one lipid, and
$28\%$ for three lipids 
are shown in Figure~\ref{fig:cacl_complexes} for \ce{Ca^{2+}}:$n\times$POPC complexes at 285~mM concetration.
\todo{Also the larger complexes were analysed, right? So we can write ''Larger complexes were not existent in simulation.'', or something like that?}
While the ternary complex had the largest propensity,
the complexes with 1~and 3~lipids occur with only a slightly lower probability,
suggesting more complex binding stoichiometry than the simple ternary complex model.
%The symmetry of the distribution of complexes,
%from simulation as shown in Figure~\ref{fig:cacl_complexes} --   
%i.e.,
It should be, however, noted that on average one Calcium binds roughly to two lipids,
...
shown in Fig.~\ref{fig:cacl_complexes}. In contrast to Calcium, the Sodium ions
most likely binds to one POPC, while the probability to bind two lipids is less likely
and very small for more than two.
...
The recent MD simulations indicate that the equilibration of \ce{Ca^{2+}}
binding to POPC bilayer may take hundreds of nanoseconds~\cite{javanainen17},
which could become an issue for the detailed binding studies with MD simulation
trajectories with the having lenght less than several microseconds \cite{catte16}.
...
To analyze the residence time in the ECC-lipid model we \todo{describe how the residence time analysis was done}.
Histograms of the residence times analyzed from the ECC-lipid simulation and from the CHARMM36 simulation data
\todo{specify the system, POPC?}
taken from the previous study \cite{javanainen17} (the simulation data was downloaded from Ref.~\citenum{zenodo.259376})
...
\todo{This paragraph should be clarified, but I cannot do it, because the residence time figure is not there yet.}
In the CHARMM36 simulation
...
Other simulations with CHARMM36 and Slipids models reported in the previous work show even longer residence times. 
The ECC-lipids model estimates 90\% of the residence times of calcium bound to a POPC membrane to be 
smaller than $60\,\mathrm{ns}$, % exactly $53\,\mathrm{ns}$
which is at least two orders of magnitude lower than for the CHARMM36.
In addition, the longest observed residence time in the ECC-lipid simulation is $120\,\mathrm{ns}$, 
...
The reduced barrier for the Calcium binding is also visible in the density distributions
in Fig.~\ref{fig:cacl-dens}, where the more shallow minimum for the Calcuim
density is observed at the interface.
