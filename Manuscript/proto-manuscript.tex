...
For example, there is a debate in the literature on the stoichiometry of \ce{Ca^{2+}} binding to POPC~\cite{Altenbach84} \todoii{REFs}{Add references on Ca2+:POPC stoichiometry}. 
In line with the early experimental finding~\cite{Altenbach84}, we find that our data fit well a ternary complex model, which assumes 2 POPC molecules per 1 \ce{Ca^{2+}}. 
...
This cannot be observed in experiments without atomistic detail as it makes only a small perturabation to the binding isotherm that assume 2~POPC per 1~\ce{Ca^{2+}}. 
...
\begin{figure}[]
  \centering
  \includegraphics[width=9.0cm,angle=0]{../Fig/CA_binding.eps}
